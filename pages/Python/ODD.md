...
- [Factory の比較](https://refactoring.guru/ja/design-patterns/factory-comparison)
  ...
  - Abstruct Factory パターン：Factory クラスの選択で生成内容を切り替える
    - これらの Factory クラスに対して抽象基底クラスを定義するから Abstruct Factory パターン
    - 抽象基底クラスの継承の代わりにダックタイピングを使っても OK

#### Factory Method パターン

- 概要：（サブクラスで）生成用のメソッドを定義する
- 使い所：
  - オブジェクト生成が複雑
  - インターフェースが共通だが生成過程が異なるオブジェクトが複数存在する
- 使用方法
  - 抽象基底クラスでは生成に関するメソッドを abstractmethod として継承時に定義させる
  - あるいは継承を使わずにクラス自体を引数として与えダックタイピングを使う(Dependency Injection)

#### Abstruct Factory

- 概要：オブジェクト生成専門のクラスを用意・
- 使い所：
  - オブジェクト生成が複雑
  - 生成過程が似ているオブジェクトが複数存在する
- 使用方法：
    1. オブジェクトの生成過程を分割
